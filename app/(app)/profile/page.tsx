...
import { Crown, Zap, LogOut, Trash2 } from 'lucide-react'
    ...
    <div className="p-6 max-w-lg mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="card p-4 text-center">
          <p className="text-3xl font-extrabold text-white">{profile?.credits ?? 0}</p>
          <p className="text-zinc-400 text-sm mt-1">Crédits restants</p>
        </div>
        <div className="card p-4 text-center">
          <p className="text-3xl font-extrabold text-white">{totalGenerations ?? 0}</p>
          <p className="text-zinc-400 text-sm mt-1">Essayages réalisés</p>
        </div>
      </div>

      {/* Plan */}
      <div className={`card p-5 mb-6 ${profile?.is_premium ? 'border-brand-500/40 bg-brand-500/5' : ''}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-white">{profile?.is_premium ? 'Premium' : 'Gratuit'}</p>
      ...
      <div className="space-y-2">
        ...
        <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-all text-sm">
